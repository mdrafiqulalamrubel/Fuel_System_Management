...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_product'])) {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO fuel_products (product_name, unit_price, purchase_rate, product_code, unit_type, is_active) 
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    
    if($stmt->execute([$product_name, $unit_price, $purchase_rate, $product_code, $unit_type, $is_active])) {
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_product'])) {
    ...
    $stmt = $pdo->prepare("
        ...
        SET product_name = ?, unit_price = ?, purchase_rate = ?, product_code = ?, unit_type = ?, is_active = ?
        WHERE id = ?
    ");
    
    if($stmt->execute([$product_name, $unit_price, $purchase_rate, $product_code, $unit_type, $is_active, $product_id])) {
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_nozzle'])) {
    ...
    $opening_meter = floatval($_POST['opening_meter'] ?? 0);
    $closing_meter = floatval($_POST['closing_meter'] ?? 0);
    
    // If pipeline, no tank_id needed
    if($is_pipeline) {
        $tank_id = null;
    }
...
<head>
    ...
    <style>
        ...
        /* ============================================= */
        /* FIXED: Tab Navigation Styles with better visibility */
        /* ============================================= */
        .nav-tabs {
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            ...
            <!-- ============================================= -->
            <!-- FIXED: Tabs with proper visibility -->
            <!-- ============================================= -->
            <ul class="nav nav-tabs" id="settingsTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link <?php echo $active_tab == 'products' ? 'active' : ''; ?>" 
                            ...
                            aria-selected="<?php echo $active_tab == 'products' ? 'true' : 'false'; ?>">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link <?php echo $active_tab == 'tanks' ? 'active' : ''; ?>" 
                            ...
                            aria-selected="<?php echo $active_tab == 'tanks' ? 'true' : 'false'; ?>">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link <?php echo $active_tab == 'nozzles' ? 'active' : ''; ?>" 
                            ...
                            aria-selected="<?php echo $active_tab == 'nozzles' ? 'true' : 'false'; ?>">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="?tab=products&delete_product_id=<?php echo $p['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this product? This will also remove it from all tanks.')">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="?tab=tanks&delete_tank_id=<?php echo $t['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this tank? All nozzles connected to it will be affected.')">
                                                    <i class="fas fa-trash"></i>
                                                </a>
                                            </td>
                                        </tr>
                                ...
                                <table class="table table-bordered table-hover" id="nozzlesTable">
                                    ...
                                    <tbody>
                                        <?php $sl=1; foreach($nozzles as $n): 
                                            $is_pipeline = $n['is_pipeline'] == 1;
                                            $total_sold = ($n['closing_meter'] ?? 0) - ($n['opening_meter'] ?? 0);
                                        ?>
                                        <tr class="<?php echo $is_pipeline ? 'pipeline-row' : ''; ?>">
                                            ...
                                            <td class="text-center">
                                                ...
                                                <?php endif; ?>
                                                <button class="btn btn-sm btn-warning" onclick="editNozzle(<?php echo $n['id']; ?>, '<?php echo addslashes($n['nozzle_name']); ?>', <?php echo $n['product_id']; ?>, '<?php echo $n['unit_type']; ?>', <?php echo $is_pipeline ? 1 : 0; ?>, '<?php echo $n['pipeline_source'] ?? ''; ?>', <?php echo $n['tank_id'] ?? 'null'; ?>, <?php echo $n['is_active']; ?>)">
                                                    <i class="fas fa-pen"></i>
                                                </button>
